			...
			<div class="albums">
				<div>
					<div class="tittle-head">
    					...
    					%>
							<h3 class="tittle"><%=category.getCatName()%></h3>
						<%}%>
						<div class="clearfix"> </div>
					</div>
					<!-- Single Blog Post -->								
	...
	<div class="blog-pagenat">
		<ul>
			<%
		    	int numberOfPages = (Integer) request.getAttribute("numberOfPages");
		       		...
		       		if (listSong != null && listSong.size() > 0) {
		       			if (currentPage != 1) {
		    %>
				<li><a class="frist" href="<%=request.getContextPath()%>/danh-muc/<%=StringUtil.makeSlug(category.getCatName())%>-<%=category.getCatId()%>/p<%=currentPage - 1%>">Prev</a></li>
			<% }
		    ...
		    %>
		    	<li><a href=""><%=i%></a></li>
		    				<%} else {%>
				<li><a href="<%=request.getContextPath()%>/danh-muc/<%=StringUtil.makeSlug(category.getCatName())%>-<%=category.getCatId()%>/p<%=i%>"><%=i%></a></li>
			<% }} %>
			<%
		    	if (currentPage != numberOfPages) {
		    %>
				<li><a class="last" href="<%=request.getContextPath()%>/danh-muc/<%=StringUtil.makeSlug(category.getCatName())%>-<%=category.getCatId()%>/p<%=currentPage + 1%>">Next</a></li>
			<%
		    	}}
		    %>
			<div class="clearfix"></div>
