...
<div class="content_resize">
  <div class="mainbar">
    ...
    <%
    	if (currentPage != 1) {
    %>
    <a href="<%=request.getContextPath()%>/danh-muc/<%=StringUtil.makeSlug(category.getCatName())%>-<%=category.getCatId()%>/p<%=currentPage - 1%>">&laquo;</a>
    <%
    	}
    %>
    <%
    ...
    %>
    <a href="<%=request.getContextPath()%>/danh-muc/<%=StringUtil.makeSlug(category.getCatName())%>-<%=category.getCatId()%>/p<%=i%>"><%=i%></a>
    <%
    	}}
    %>
    <%
    ...
    %>
    <a href="<%=request.getContextPath()%>/danh-muc/<%=StringUtil.makeSlug(category.getCatName())%>-<%=category.getCatId()%>/p<%=currentPage + 1%>">&raquo;</a>
    </p>
    <%
    	}}
    %>
